    ...
    <AnimatedSection id="skills">
      ...
      <div className="max-w-2xl flex flex-col gap-3">
        {skillCategories.map((cat) => (
          <div key={cat.label} className="anim-item glow-card rounded-lg px-4 py-3 bg-card">
            ...
            <div className="flex flex-wrap gap-2">
              {cat.skills.map((skill) => (
                <span
                  key={skill}
                  className="glow-pill text-xs sm:text-sm px-3 py-1 rounded-full bg-secondary text-foreground"
                >
                  {skill}
                </span>
              ))}
